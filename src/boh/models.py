...
class Application(TimeStampedModel, models.Model):
    ...
    requestable = models.BooleanField(default=True, help_text=_('Specify if activities can be externally requested for this application.'))
...
class Engagement(TimeStampedModel, models.Model):
    ...
    version = models.CharField(max_length=16, blank=True, help_text=_('The software version used during this engagement.'))
    description = models.TextField(blank=True)
...
class EngagementRequest(TimeStampedModel, models.Model):
    """An request to create an engagement."""
    ...
    description = models.TextField(blank=True)
    status = models.CharField(max_length=8, choices=STATUS_CHOICES, default=PENDING_STATUS)
    version = models.CharField(max_length=16)
    note = models.CharField(max_length=30, blank=True)

    requester = models.ForeignKey(Person)
    reviewer = models.ForeignKey(settings.AUTH_USER_MODEL, blank=True, null=True)
    application = models.ForeignKey(Application)
    engagement = models.ForeignKey(Engagement, blank=True, null=True)
    activity_types = models.ManyToManyField(ActivityType)

    objects = managers.EngagementRequestManager.from_queryset(managers.EngagementRequestQuerySet)()
    ...
    def is_pending(self):
        return self.status == EngagementRequest.PENDING_STATUS

    def is_accepted(self):
        return self.status == EngagementRequest.ACCEPTED_STATUS

    def is_declined(self):
        return self.status == EngagementRequest.DECLINED_STATUS


class EngagementRequestSubscription(TimeStampedModel, models.Model):
    """Creates a link between external people or groups and service requests."""
    ...
    active = models.BooleanField(default=True, db_index=True, help_text=_('Toggles use of this subscription.'))
    ...
    # Send engagement closed? Send engagement results?

    engagement_request = models.ForeignKey(EngagementRequest)

    objects = managers.EngagementRequestSubscriptionManager.from_queryset(managers.EngagementRequestSubscriptionQuerySet)()

    class Meta:
        ordering = ['name']

    def __str__(self):
        return str(self.engagement_request) + ' - ' + self.name

    def is_active(self):
        return self.active


class EngagementRequestComment(TimeStampedModel, models.Model):
    """Comment for a specific service request."""
    message = models.TextField()

    engagement_request = models.ForeignKey(EngagementRequest)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, blank=True, null=True)
    subscription = models.ForeignKey(EngagementRequestSubscription, blank=True, null=True)
