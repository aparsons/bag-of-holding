...
class EngagementRequestManager(models.Manager):
    pass


class EngagementRequestQuerySet(models.QuerySet):
    def accepted(self):
        """Returns Service Requests with an accepted status."""
        from .models import EngagementRequest
        return self.filter(status=EngagementRequest.ACCEPTED_STATUS)

    def declined(self):
        """Returns Service Requests with a declined status."""
        from .models import EngagementRequest
        return self.filter(status=EngagementRequest.DECLINED_STATUS)

    def last_thirty_days(self):
        """Returns Service Requests created in the last 30 days."""
        date_delta = timezone.now() - datetime.timedelta(days=30)
        return self.filter(created_date__gte=date_delta)

    def pending(self):
        """Returns Service Requests with a pending status."""
        from .models import EngagementRequest
        return self.filter(status=EngagementRequest.PENDING_STATUS)

    def resolved(self):
        """Returns Service Requests with an accepted or declined status."""
        from .models import EngagementRequest
        return self.filter(Q(status=EngagementRequest.ACCEPTED_STATUS) | Q(status=EngagementRequest.DECLINED_STATUS))


class EngagementRequestSubscriptionManager(models.Manager):
    pass


class EngagementRequestSubscriptionQuerySet(models.QuerySet):
    def active(self):
        """Returns Engagement Request Subscriptions that are active."""
        return self.filter(active=True)

    def inactive(self):
        """Returns Engagement Request Subscriptions that have been deactivated."""
