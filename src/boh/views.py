...
def dashboard_requests(request):
    pending_requests = models.EngagementRequest.objects.pending()
    closed_requests = models.EngagementRequest.objects.last_thirty_days().resolved()

    return render(request, 'boh/dashboard/requests/list.html', {
...
@login_required
@require_http_methods(['GET'])
def dashboard_requests_detail(request, engagement_request_id):
    engagement_request = get_object_or_404(models.EngagementRequest, pk=engagement_request_id)
    comment_form = forms.EngagementRequestCommentForm()

    RequestSubscriptionsFormSet = inlineformset_factory(
        models.EngagementRequest,
        models.EngagementRequestSubscription,
        fields=('name', 'email', 'label', 'send_status_updates', 'send_comments'),
        extra=1,
    )

    subscriptions_formset = RequestSubscriptionsFormSet(request.POST or None, instance=engagement_request)

    return render(request, 'boh/dashboard/requests/detail.html', {
        'engagement_request': engagement_request,
        'comment_form': comment_form,
        'subscriptions_formset': subscriptions_formset,
        'active_top': 'dashboard',
        'active_tab': 'requests'
    })
...
def dashboard_requests_edit(request, engagement_request_id):
    engagement_request = get_object_or_404(models.EngagementRequest, pk=engagement_request_id)
    ...
    return render(request, 'boh/dashboard/requests/edit.html', {
        'engagement_request': engagement_request,
        'active_top': 'dashboard',
        'active_tab': 'requests'
    })
